...
class YahooMacroScraper:
            ...
            for yf_symbol, internal_code in self.tickers.items():
                try:
                    ...
                    if len(ticker_list) > 1:
                        if yf_symbol not in df_raw.columns.get_level_values(1): continue
                    ...
                    df_target.rename(columns={'Date': 'trade_date', 'Close': 'close', 'Open': 'open', 'High': 'high', 'Low': 'low', 'Volume': 'volume'}, inplace=True)
                    ...
                    # 格式化并入库
                    clean_df = UsMacroDailyKlineModel.format_dataframe(df_target, default_ticker=internal_code)
                    if not clean_df.empty:
                        self.repo.insert_macro_daily_klines(clean_df)
                        app_logger.info(f"✅ Yahoo: {internal_code} 同步完成 ({len(clean_df)} 行)。")
    ...
    def start(self):
        ...
        # 1. 增量探测并运行
        last_date_str = self.repo.get_latest_trade_date_in_macro_daily_klines(list(self.tickers.values()))
