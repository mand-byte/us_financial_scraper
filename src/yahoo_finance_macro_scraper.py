...
class YahooMacroScraper:
    ...
    def fetch_and_save(self, start_date="2014-01-01"):
        app_logger.info(
            f"📡 正在从 Yahoo Finance 拉取宏观日线数据 (起点: {start_date})..."
        )
        ...
        try:
            ...
            for yf_ticker, indicator_code in self.tickers.items():
                ...
                if yf_ticker_lower not in df_close.columns:
                    continue
                ...
                df_to_save = UsMacroDailyKlineModel.format_dataframe(
                    df_single, default_ticker=indicator_code
                )
    ...
    def _checking_data_complementation(self):
        try:
            ...
            res = MarketDataRepo().get_latest_trade_date_in_macro_daily_klines(
