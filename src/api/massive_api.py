...
class MassiveApi:
                    ...
                    result_raw.append(
                        {
                            ...
                            "delisted_date": item.get("delisted_utc"),
                        }
                    )
                # 检查有没有下一页
            ...
            while True:
                ...
                for item in results:
                    result_raw.append({
                        ...
                        "low": item.get("l"),
                        "volume": item.get("v"),
                        "trades_count": item.get("n"),
                        "timestamp": item.get("t"),
                        "vwap": item.get("vw"),
                    })
                
                # 立即产出当前页数据，让调用方写入并清理内存
                yield pd.DataFrame(result_raw)
    ...
    def get_stock_news(
        ...
        # 可写published_utc.gte，published_utc.gt，published_utc.lte，published_utc.lt
        published_utc_type: str = "published_utc",
    ...
    def get_balance_sheets(
        ...
        limit: int = 50000,
        filing_date: Optional[str] = "2009-03-29",
    ) -> pd.DataFrame:
        """
        ...
        try:
            ...
            while True:
                for item in data_.get("results", []):
                    ...
                    row = {
                        ...
                        "total_current_liabilities": item.get(
            ...
            app_logger.error(
                f"抓取资产负债表数据失败 [cik: {cik}] [filing_date: {filing_date}]: {e}"
            )
            return pd.DataFrame()
    ...
    def get_cashflow_statements(
        ...
        try:
            ...
            while True:
                for item in data_.get("results", []):
                    ...
                    row = {
                        ...
                        "net_cash_from_operating_activities": item.get(
                            "net_cash_from_operating_activities"
                        ),
                        "purchase_of_property_plant_and_equipment": item.get(
            ...
            app_logger.error(
                f"抓取现金流量表数据失败 [cik: {cik}] [filing_date: {filing_date}]: {e}"
            )
            return pd.DataFrame()
    ...
    def get_income_statements(
        ...
        try:
            ...
            app_logger.error(
                f"抓取利润表数据失败 [cik: {cik}] [filing_date: {filing_date}]: {e}"
            )
            return pd.DataFrame()
    ...
    def get_ratios(
        ...
        try:
            ...
            app_logger.error(f"抓取 Ratios 数据失败 [cik: {cik}]: {e}")
            return pd.DataFrame()
    ...
    def get_float(self, ticker: str, limit: int = 5000) -> pd.DataFrame:
        ...
        try:
            ...
            app_logger.error(f"抓取 float 数据失败 [ticker: {ticker}]: {e}")
            return pd.DataFrame()
    ...
    def get_dividens(
        ...
        try:
            ...
            app_logger.error(
                f"抓取 dividends 数据失败 [ticker: {ticker}] [ex_dividend_date: {ex_dividend_date}]: {e}"
            )
            return pd.DataFrame()
